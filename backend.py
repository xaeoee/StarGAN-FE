...
async def generate_emotion(item: Item):
    ...
    try:
        ...
        print("Loading model from:", G_path)
        saved_checkpoint_G = torch.load(G_path, map_location=torch.device(device))
        G = Generator(conv_dim, c_dim, 6)
        G.to(device)
        G.load_state_dict(saved_checkpoint_G, strict=False)
        ...
        if img_list is None:
            print("detection_and_resize_original returned None")
            return {"message": "얼굴이 인식되지 않았습니다."}
        
        if len(img_list) == 0:
            print("No face detected in the image")
            return {"message": "이미지에 얼굴이 인식되지 않습니다."}
        
        print(f"img_list is not None and has {len(img_list)} items")
        ...
        x_real = transform(img)
        print(f"Image shape: {x_real.shape}")
        x_real = x_real.view(1, 3, image_size, image_size)
        c_org = torch.Tensor([3])
        ...
        with torch.no_grad():
            x_real = x_real.to(device)
            c_trg_list = create_labels(c_org, c_dim)
            print(f"Target list created with {len(c_trg_list)} elements")
            x_fake_list = [x_real]
            x_origin_list = [original]
            x_mesh_list = [original]

            for c_trg in c_trg_list:
                print(f"Generating image for target: {c_trg}")
                x_fake_list.append(G(x_real, c_trg))
                x_origin_list.append(torch.tensor(original))
                x_mesh_list.append(torch.tensor(original))
            ...
            images_data['images'] = []
            for i, fake in enumerate(x_fake_list):
                print(f"Processing fake image {i}")
                translate_img = fake.data.cpu().squeeze(0)
                translate_img = translate_img.permute(1, 2, 0).numpy()
                translate_img = cv2.resize(translate_img,(w, h))
                translate_img = torch.from_numpy(translate_img).permute(2, 0, 1)
                print(f"Translate image min: {translate_img.min()}, max: {translate_img.max()}")
                for j in range(3):
                    for k in range(y, y+h):
                        x_origin_list[i][j][k][x:x+w] = translate_img[j][k-y]
                face_dict, mesh_img = get_face_mesh(to_pil_image(0.5*x_origin_list[i] + 0.5))
                print(f"Face mesh detected: {face_dict}")
                
                if face_dict is None:
                    print(f"No face mesh detected for image {i}, skipping mesh overlay")
                    numpy_image = denorm(x_origin_list[i].data.cpu()).numpy()
                    numpy_image = np.transpose(numpy_image, (1, 2, 0))
                    pil_image = Image.fromarray((numpy_image*255).astype(np.uint8))
                    buffered = BytesIO()
                ...
                if isinstance(face_dict, dict):
                    print(f"face_dict is a dict with {len(face_dict)} items")
                else:
                    print(f"face_dict is not a dict, it is: {type(face_dict)}")
                ...
                numpy_image = denorm(x_mesh_list[i].data.cpu()).numpy()
                numpy_image = np.transpose(numpy_image, (1, 2, 0))
                print(f"Generated image {i} shape: {numpy_image.shape}")
                pil_image = Image.fromarray((numpy_image*255).astype(np.uint8))
                buffered = BytesIO()
                pil_image.save(buffered, format="JPEG")
                img_byte = buffered.getvalue()
        ...
        print(f"Error in generate_emotion: {str(e)}")
        raise HTTPException(status_code=400, detail=f"Error during emotion generation: {str(e)}")

@app.post("/test")
async def test_endpoint(item: Item):
    try:
        # 간단히 이미지 데이터 길이만 확인
        image_length = len(item.image)
        return {"message": f"Received image data of length: {image_length}"}
    except Exception as e:
        logging.error(f"Error in test_endpoint: {str(e)}")
        raise HTTPException(status_code=400, detail=str(e))